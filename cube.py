...
class Cube:
    ...
    def draw(self):
        ...
        for surface in self.surfaces:
            glMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, [
                       *self.colors[i], 1])  # geel materiaal, let op alpha
            glMaterial(GL_FRONT_AND_BACK, GL_SPECULAR,
                       [1, 1, 1])  # witte reflectie
            # groote van glimvlek
        ...
        if(self.check):
            glMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, [
                       1, 1, 1, 1])  # geel materiaal, let op alpha
        else:
            glMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, [
                       0, 0, 0, 1])  # geel materiaal, let op alpha
    ...
    def setRotate(self, direction, x, y, z):
        if(x != 0):
            #all in front become all in bottom
            self.cubePos[2], self.cubePos[1] = self.cubePos[1], (self.cubePos[2]*x*-1)
            self.rotationOrder.append('x')
            if(x == -1):
                self.rotationOrder.append('x')
                self.rotationOrder.append('x')
        if(y == 1):
            self.cubePos[2], self.cubePos[0] = self.cubePos[0], (self.cubePos[2]*x*-1)
